      ...
      <div className="max-w-6xl pl-8">
        ...
        <header className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold bg-gradient-to-r from-green-400 to-green-500 text-transparent bg-clip-text mb-2">
                Project Docs
              </h2>
...
const TemplateModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 bg-gray-500/50 dark:bg-black/80 bg-white dark:bg-black flex items-center justify-center z-50">
      <div className="relative p-6 rounded-md bg-white dark:bg-black w-full max-w-2xl
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold bg-gradient-to-r from-green-400 to-green-500 text-transparent bg-clip-text">
              Choose a Template
            </h3>
          ...
          <div className="grid grid-cols-1 gap-3">
            {templates.map(([key, template]) => (
              <button
                ...
                disabled={isCreating}
                className="p-4 text-left border border-gray-200 dark:border-gray-700 rounded-lg hover:border-green-400 hover:bg-green-50 dark:hover:bg-green-900/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
              >
                <div className="flex items-start gap-3">
                  <span className="text-2xl flex-shrink-0 group-hover:scale-105 transition-transform">{template.icon}</span>
...
const KnowledgeSection: React.FC<{
  ...
  const colorMap = {
    blue: {
      ...
      buttonHover: 'hover:bg-green-500/30',
      buttonBorder: 'border-green-500/40',
      buttonShadow: 'hover:shadow-[0_0_15px_rgba(#50c878,0.3)]'
    },
    green: {
      bg: 'bg-green-500/10',
      border: 'border-green-500/30',
    ...
    green: {
      ...
      buttonHover: 'hover:bg-green-500/30',
      buttonBorder: 'border-green-500/40',
      buttonShadow: 'hover:shadow-[0_0_15px_rgba(80,200,120,0.3)]'
    },
    orange: {
      bg: 'bg-orange-500/10',
  ...
  return <section>
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-bold text-gray-800 dark:text-white flex items-center">
          <span className={`w-2 h-2 rounded-full bg-${color}-400 shadow-[0_0_8px_rgba(#50c878,0.6)] mr-2`} />
          {title}
        </h3>
      ...
      <div className={`bg-white/10 dark:bg-black/30 border ${colorMap[color].border} rounded-lg p-4 bg-white dark:bg-black relative overflow-hidden min-h-[200px]`}>
        <div className="absolute top-0 left-0 right-0 h-[1px] bg-green-500/30"></div>
        {sources && sources.length > 0 ? <div className="space-y-3">
            {sources.map(source => source && <div key={source.id} className="flex items-center gap-3 p-2 rounded-md bg-white/10 dark:bg-black/30 border border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 transition-all">
                    {source.type === 'url' ? <LinkIcon className="w-4 h-4 text-green-600 dark:text-green-400" /> : <Upload className="w-4 h-4 text-green-600 dark:text-green-400" />}
...
const SourceSelectionModal: React.FC<{
  ...
  return <div className="fixed inset-0 bg-gray-500/50 dark:bg-black/80 bg-white dark:bg-black flex items-center justify-center z-50">
      <div className="relative p-6 rounded-md bg-white dark:bg-black w-full max-w-3xl
          bg-white dark:bg-black dark:from-white/10 dark:to-black/30
          border border-gray-200 dark:border-gray-200 dark:border-gray-800
          shadow-sm dark:shadow-sm
          before:content-[''] before:absolute before:top-0 before:left-0 before:right-0 before:h-[2px] 
        ...
        <div className="relative z-10">
          ...
          <div className="flex gap-3 mb-4">
            <div className="relative flex-1">
              ...
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search sources..." className="w-full bg-white/50 dark:bg-black/70 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white rounded-md py-2 pl-10 pr-3 focus:outline-none focus:border-green-400 focus:shadow-[0_0_10px_rgba(#50c878,0.2)] transition-all duration-300" />
            </div>
            <Button onClick={onAddSource} variant="primary" accentColor="blue" className="shadow-lg shadow-green-500/20">
          ...
          <div className="bg-white/50 dark:bg-black/50 border border-gray-200 dark:border-gray-800 rounded-md p-2 max-h-[50vh] overflow-y-auto mb-6">
            {filteredSources.length > 0 ? <div className="space-y-2">
                {filteredSources.map(source => <div key={source.id} onClick={() => onToggleSource(source.id)} className={`flex items-center gap-3 p-3 rounded-md cursor-pointer transition-all duration-200 
                      ${selectedSources.includes(source.id) ? 'bg-green-100/80 dark:bg-green-900/30 border border-green-300 dark:border-green-500/50' : 'bg-white/50 dark:bg-black/30 border border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700'}`}>
                    <div className={`w-5 h-5 rounded-md border flex items-center justify-center 
                        ${selectedSources.includes(source.id) ? 'bg-green-500 border-green-500' : 'border-gray-400 dark:border-gray-600'}`}>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button onClick={onSave} variant="primary" accentColor="blue" className="shadow-lg shadow-green-500/20">
